"""The Wizard of Oz Panel.

This file contains the code required to represent
the Wizard of Oz Wrap panel in the GUI.
...
class TheWizardOfOzPanel(tk.Frame):
    """The Wizard of Oz Panel class.

    This class builds the representation of the
    Wizard of Oz Panel in the GUI.

    Author: Dustin Hayes djhayes@ksu.edu
    Version 0.1
    """
    def __init__(self, master, item: TheWizardOfOz = None,
                 buttons: bool = True) -> None:
        """The Wizard of Oz Panel constructor.

        This constructor sets the state of our
        representation to match the state of
        the Wizard of Oz object.  It makes
        ...
        Args:
            ...
            item: Reference of WizardOfOz object
            used to instantiate the panel.
        ...
        chicken = tk.Checkbutton(master=self, text="Corned Beef",
                                 variable=self._chicken, onvalue=True,
                                 offvalue=False)
        chicken.grid(row=2, column=1, padx=2, pady=2, sticky="W")

        self._spinach = tk.BooleanVar(value=bool(self._item.spinach))
        spinach = tk.Checkbutton(master=self, text="Spinach",
                                 variable=self._spinach, onvalue=True,
                                 offvalue=False)
        spinach.grid(row=3, column=1, padx=2, pady=2, sticky="W")
        ...
        cheese = tk.Checkbutton(master=self, text="Cheese",
                                ...
                                offvalue=False)
        cheese.grid(row=4, column=1, padx=2, pady=2, sticky="W")

        addlabel = tk.Label(master=self, text="Addins")
        addlabel.grid(row=5, column=1, padx=2, pady=2, sticky="EW")
    ...
    def action_performed(self, text, combo: bool = False):
        ...
        if text == "save":
            ...
            self._item.spinach = self._spinach.get()
