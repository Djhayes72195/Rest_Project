"""The West Side Story Panel.

This file contains the code required to represent
the West Side Story Wrap panel in the GUI.
...
class WestSideStoryPanel(tk.Frame):
    """The West Side Story Panel class.

    This class builds the representation of the
    West Side Story Panel in the GUI.

    Author: Dustin Hayes djhayes@ksu.edu
    Version 0.1
    """
    def __init__(self, master, item: WestSideStory = None, buttons: bool = True) -> None:
        """The West Side Story Panel constructor.

        This constructor sets the state of our
        representation to match the state of
        the West Side Story object.  It makes
        ...
        corned_beef = tk.Checkbutton(master=self, text="Corned Beef",
                                     ...
                                     offvalue=False)
        corned_beef.grid(row=2, column=1, padx=2, pady=2, sticky="W")

        self._cabbage = tk.BooleanVar(value=bool(self._item.cabbage))
        cabbage = tk.Checkbutton(master=self, text="Cabbage",
                                 variable=self._cabbage, onvalue=True,
                                 offvalue=False)
        cabbage.grid(row=3, column=1, padx=2, pady=2, sticky="W")
    ...
    def action_performed(self, text: str, combo: bool = False) -> None:
        ...
        if text == "save":
            ...
            self._item.cabbage = self._cabbage.get()
